# Cartpole by Dueling DQN using pyTorch
...
class NeuralNetwork(nn.Module):
    def __init__(self,input_size,output_size):
        ...
        # 状态值网络部分
        self.staval_stack = nn.Sequential(
                nn.Linear(20,20),
                nn.ReLU(),
                nn.Linear(20,1),
                )
    ...
    def forward(self, x):
        ...
        stv = self.staval_stack(pub)
        Q = stv+adv-adv.mean(1).unsqueeze(1).expand(
                x.size(0),self.output_size)
...
class DuelingDQN():
    ...
    def train_Q_network(self):        
        ...
        for x in minibatch:     # 对minibatch中每一条MDP数据循环
            if x[4]:            # 如果已经到达终止状态
                ...
                temp = torch.from_numpy(x[3]).unsqueeze(0).to(torch.float32)
                                # 用当前Q-网络计算下一状态的动作值
                value = self.Q_network.forward(temp) 
                                # 求最大动作值对应的动作
                action_max = torch.argmax(value).item() 
                action_max_onehot = torch.eye(self.env.aspace_size
                                              )[action_max]
                                # 用目标Q-网络计算动作值
                value_next = self.Q_network_t(temp).squeeze(0) 
                                # 贪婪动作的动作值
        ...
        # 预测批量值和损失函数
        pred = torch.sum(torch.multiply(self.Q_network.forward(state_batch),
                                        action_batch),dim=1)
    ...
    ## 训练函数
    def train(self,num_episodes=450,num_steps=1000):
        # 外层循环指导最大轮次
        rewards = []                    # 每一轮次的累积奖励
...
if __name__ == '__main__':
    ...
    agent = DuelingDQN(env)         # 创建一个DDQN智能体
    agent.train()                   #训练智能体
    agent.test()                    # 测试智能体