# -*- coding: utf-8 -*-
"""
Created on Mon Apr 18 15:37:27 2022
...
class Worker(mp.Process):
    def __init__(self,id,device,env,beta,global_network,global_optimizer,
                 global_T,global_T_MAX,t_MAX,
                 global_episode,global_return_display,
                 global_return_record,global_return_display_record):
        super().__init__()
        self.id = id                # 分布智能体ID
        self.device = device        # 计算设备
        self.env = env              # 环境模型
        self.beta = beta            # 交叉熵系数
                                    # 定义分布智能体A-C网络
        self.local_network = ACNet(env.obs_dim,env.act_dim,
                                   env.act_limit,self.device
                                   ).to(self.device)
        self.global_network = global_network        # 中心智能体A-C网络
        self.global_optimizer = global_optimizer    # 中心智能体优化器
    ...
    def update_global(self,states,actions,rewards,
        ...
        # 损失函数
        loss = self.local_network.loss_func(states,actions,v_t,self.beta) 
    ...
    def run(self): 
        ...
        episode_return = 0                      # 回合回报

        # 循环，直到规定的全局交互次数
        while self.global_T.value <= self.global_T_MAX:
            # 获取一个回合的交互数据，即一个buffer数据
            t_start = t
            buffer_states = []
            buffer_actions = []
            buffer_rewards = []
            ...
            while not done and t-t_start != self.t_MAX:
                action = self.local_network.select_action(
            ...
            if done:
                ...
                with self.global_episode.get_lock():
                    self.global_episode.value += 1      # 更新全局回合次数
                    self.global_return_record.append(episode_return)
                    if self.global_episode.value == 1:  # 完成第一个回合
...
#    mp.set_start_method('spawn')
    ...
    # 定义分布智能体
    workers = [Worker(i,device,env,beta,global_network,optimizer,
